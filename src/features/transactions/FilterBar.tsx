    ...
    <div className="space-y-2">
      ...
      <div className="flex items-center gap-2">
        ...
        >
          ...
          {activeFilterCount > 0 && (
            <span className="absolute -right-1 -top-1 flex h-4 w-4 items-center justify-center rounded-full bg-[hsl(var(--color-blue))] text-[9px] font-bold text-white">
              {activeFilterCount}
            </span>
          )}
        </Button>
                ...
                .map((cat) => {
                  ...
                  return (
                    <FilterPill
                      ...
                      label={cat.name}
                      colorClass={filters.categoryId === cat.id ? c.solid : ''}
                    />
                  )
                })}
            </div>
...
function FilterChip({ label, onClear }: { label: string; onClear: () => void }) {
  return (
    <button
      onClick={onClear}
      className="inline-flex items-center gap-1 rounded-md bg-[hsl(var(--color-blue-soft))] px-2 py-0.5 text-xs font-medium text-[hsl(var(--color-blue))]"
    >
      {label}
      <X className="h-3 w-3" />
    </button>
